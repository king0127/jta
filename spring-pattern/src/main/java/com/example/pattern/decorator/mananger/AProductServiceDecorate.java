...
@Slf4j
@Service
@Decorate(scene = ConstantCommon.TASK_TYPE, type = ConstantCommon.TASK_TYPE_PPAP)
public class AProductServiceDecorate extends AbstractHandler<String, Integer> {

    @Override
    public Integer handle(String type) {

        log.info(" 【PPAP】 装饰器模式，在执行公共方法之前， 执行一些校验数据类型 ");
        CertifcatePO data = makeData();
        baseHandler.handle(data);

        log.info(" 【PPAP】 执行公共方法之后， 处理一些额外其他操作---------- ");
